...
# working dir
wdir_fg = "/Users/francescogemignani/Desktop/Risk-of-Business-Failure/dataset"
...
```{r}
# L'assolutizzazione del dataset ha fatto perdere i tipi di alcune features, quindi li ripristino
aida[aida$Legal.form == "",c('Legal.form')] <- NA
aida$Legal.form <- as.factor(aida$Legal.form)
aida$ATECO.2007code <- as.factor(aida$ATECO.2007code)
aida$Legal.status <- as.factor(aida$Legal.status)
aida$Province <- as.factor(aida$Province)
aida$Comune.ISTAT.code <- as.factor(aida$Comune.ISTAT.code)
aida$Region <- as.factor(aida$Region)

aida$Company.name <- as.character(aida$Company.name)
aida$Tax.code.number <- as.character(aida$Tax.code.number)
aida$File <- as.character(aida$File)

# Esporto il dataset in formato binario
save(aida, file="aida-abs.RData")
```

```{r}
#load(file = "/Users/francescogemignani/Google Drive/SMD/aida-abs.RData")
skim(aida)
```
