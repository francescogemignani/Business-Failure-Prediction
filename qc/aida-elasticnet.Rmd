...
data <- subset(data, select = -c(Year, ROA, Net.financial.positionth, Cash.Flowth, Current.ratio))
```
...
# predicted confidence on validation set
elas.2013.pred = predict(elasticFit, newx = data.matrix(x.2013.test), type="class", s = "lambda.1se")
elas.2013.prob = predict(elasticFit, newx = data.matrix(x.2013.test), type="response", s = "lambda.1se") #as predict proba
# built-in metrics
confusionMatrix(factor(elas.2013.pred), factor(y.2013.test), positive="1", mode = "prec_recall")
```
...
par(pty="s")
roc(aidaTest2013$Failed, elas.2013.prob, plot=TRUE, legacy.axes=TRUE,
      xlab="False Positive Rate", ylab="True Positive Rate", print.auc=TRUE, col="red")
roc(aidaTest$Failed, elas.prob, plot=TRUE, legacy.axes=TRUE, add=TRUE, col="blue",
