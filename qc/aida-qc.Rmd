...
aidaTest2014 = aidaTest[aidaTest$Year == 2014]
aidaTest2015 = aidaTest[aidaTest$Year == 2015]
...
Valutazione delle performance sul test set (entrambi gli anni, 2014, 2015)
```{r}
...
lr.pred2014 = predict(lr.fit, newdata = aidaTest2014, type="raw")
lr.prob2014 = predict(lr.fit, newdata = aidaTest2014, type="prob") 
lr.pconf2014 = lr.prob2014[,2]

confusionMatrix(lr.pred2014, aidaTest2014$Failed, positive="1", mode = "prec_recall")

lr.pred2015 = predict(lr.fit, newdata = aidaTest2015, type="raw")
lr.prob2015 = predict(lr.fit, newdata = aidaTest2015, type="prob") 
lr.pconf2015 = lr.prob2015[,2]

confusionMatrix(lr.pred2015, aidaTest2015$Failed, positive="1", mode = "prec_recall")
```


Calibration plot
```{r}
cal_data2014 = calibration(aidaTest2014$Failed ~ lr.pconf2014, class="1")
cal_data2015 = calibration(aidaTest2015$Failed ~ lr.pconf2015, class="1")
cal_data = calibration(aidaTest$Failed ~ lr.pconf, class="1")

plot(cal_data2014$data$midpoint, cal_data2014$data$Percent, type="o", xlab="Bin Midpoint", ylab="Observed Event Percentage")

points(cal_data2015$data$midpoint, cal_data2015$data$Percent, col="red", pch="*")
lines(cal_data2015$data$midpoint, cal_data2015$data$Percent, col="red")

points(cal_data$data$midpoint, cal_data$data$Percent, col="orange", pch="+")
lines(cal_data$data$midpoint, cal_data$data$Percent, col="orange")

#Perfettamente calibrato
abline(a=0, b=1, lwd=0.5, lty=2)

legend("topleft", legend=c("2014","2015","2014+2015", "Perfectly Calibrated"),
       col=c("black","red","orange", "black"),
        pch=c("o","*","+",""),lty=c(1,2,3,2))
...
lr.prediction2014 = prediction(lr.pconf2014, aidaTest2014$Failed)
lr.prediction2015 = prediction(lr.pconf2015, aidaTest2015$Failed)
...
par(pty="s")
roc(aidaTest2014$Failed, lr.pconf2014, plot=TRUE, legacy.axes=TRUE,
      xlab="False Positive Rate", ylab="True Positive Rate", print.auc=TRUE, col="blue")
roc(aidaTest2015$Failed, lr.pconf2015, plot=TRUE, legacy.axes=TRUE, add=TRUE, col="red",
      xlab="False Positive Rate", ylab="True Positive Rate", print.auc=TRUE, print.auc.y=0.45)
roc(aidaTest$Failed, lr.pconf, plot=TRUE, legacy.axes=TRUE, add=TRUE, col="orange",
      xlab="False Positive Rate", ylab="True Positive Rate", print.auc=TRUE, print.auc.y=0.4)

legend("bottomright", legend=c("2014","2015","2014+2015"),
       col=c("blue","red","orange"), lwd=4)
par(pty="m")
```
...
```{r}
lr.fit = glm(best.formula, data=aidaTrain, family=binomial("logit"))
