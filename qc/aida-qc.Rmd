...
order = data[order(data$Company.name)]
View(order[1:100,])
...
```{r}
order$Age <- NA
unique.names = unique(order[order$Failed==1]$Company.name)

for(name in unique.names){
  rows = order[order$Company.name == name]
  last.year = max(rows$Year)
  
  order$Failed[order$Company.name == name & order$Year != last.year] = 0
  #print(last.year)
