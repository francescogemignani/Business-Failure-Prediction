...
# predicted confidence on validation set
elas.pred = predict(elasticFit, newx = data.matrix(x.test), type="class")
elas.prob = predict(elasticFit, newx = data.matrix(x.test), type="response") #as predict proba
# built-in metrics
confusionMatrix(data.matrix(elas.pred), data.matrix(y.test), positive="1", mode = "prec_recall")
```
