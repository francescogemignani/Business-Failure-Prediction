...
n.obs <- length(aida.yy$Age)
sqrt(n.obs)
# Compute the Freedman-Diaconis bin width
fdw <- (2*IQR(aida.yy$Age)) / (n.obs^(1/3))

# Compute the number of bins
n.bins <- (max(aida.yy$Age) - min(aida.yy$Age)) / fdw
n.bins <- as.integer(n.bins)

table(cut(aida.yy$Age, breaks = n.bins))
...
Company ages summary
```{r}
# Clean enviroment and import aida dataset for size
rm(list = ls())
