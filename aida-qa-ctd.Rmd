...
```{r}
# Filtering ages of all failed companies
aida.failed <- aida.size[aida.size$Failed=='Yes',]
...
ggplot(data=aida.failed, aes(x=Size, group=as.factor(Last.accounting.closing.date), fill=as.factor(Last.accounting.closing.date))) +
    geom_density(adjust=1.5, alpha=.4) +
    theme_ipsum() + 
...
```{r}
age.y1.failed <- aida.failed[aida.failed$Last.accounting.closing.date == 2017,'Size']
age.y2.failed <- aida.failed[aida.failed$Last.accounting.closing.date == 2018,'Size']
...
# RISULTATO: H0 non significative p-value molto basso. Considero l'ipotesi bilaterale maggiore con p-value = 1
# La media della dimensione delle aziende fallite nel 2017 è superiore alla media della dimensione delle aziende fallite nel 2018.
# Quindi le aziende fallite nel 2017 sono più grandi di quelle fallite nel 2018.
```
...
for(form.name in company.forms){
  print("**************************")
  print(form.name)
  roi.y1.failed <- aida.failed[aida.failed$Last.accounting.closing.date == 2017 & aida.failed$Legal.form==form.name ,'Size']
  roi.y2.failed <- aida.failed[aida.failed$Last.accounting.closing.date == 2018 & aida.failed$Legal.form==form.name ,'Size']
...
for(region.name in company.regions){
  print("**************************")
  print(region.name)
  roi.y1.failed <- aida.failed[aida.failed$Last.accounting.closing.date == 2017 & 
                                     aida.failed$`Registered.office.address.-.Region`==region.name ,'Size']
  roi.y2.failed <- aida.failed[aida.failed$Last.accounting.closing.date == 2018 & 
                                     aida.failed$`Registered.office.address.-.Region`==region.name ,'Size']
...
# Filtering ages of all failed companies
aida.failed <- aida.roi[aida.roi$Failed=='Yes',]
...
names(aida.failed)[names(aida.failed) == "Return.on.investment.(ROI).(%)%Last.avail..yr"] <- "Roi"
...
```{r}
# density of roi of failed companies over years selected
ggplot(data=aida.failed, aes(x=Roi, group=as.factor(Last.accounting.closing.date), fill=as.factor(Last.accounting.closing.date))) +
    geom_density(adjust=1.5, alpha=.4) +
    theme_ipsum() + 
...
```{r}
roi.y1.failed <- aida.failed[aida.failed$Last.accounting.closing.date == 2017,'Roi']
roi.y2.failed <- aida.failed[aida.failed$Last.accounting.closing.date == 2018,'Roi']
...
for(form.name in company.forms){
  print("**************************")
  print(form.name)
  roi.y1.failed <- aida.failed[aida.failed$Last.accounting.closing.date == 2017 & aida.failed$Legal.form==form.name ,'Roi']
  roi.y2.failed <- aida.failed[aida.failed$Last.accounting.closing.date == 2018 & aida.failed$Legal.form==form.name ,'Roi']
...
for(region.name in company.regions){
  ...
  print(region.name)
  roi.y1.failed <- aida.failed[aida.failed$Last.accounting.closing.date == 2017 & 
                                     aida.failed$`Registered.office.address.-.Region`==region.name ,'Roi']
  roi.y2.failed <- aida.failed[aida.failed$Last.accounting.closing.date == 2018 & 
                                     aida.failed$`Registered.office.address.-.Region`==region.name ,'Roi']
